...
it('map([1,2], "function") should throw', () => {
  let error
  try {
    map([1,2], "function")
  } catch (e) {
...
it('map([1,2], "function") should throw a TypeError with message "The second parameter must be an array"', () => {
  let error
  try {
    map([1,2], "function")
  } catch (e) {
  ...
  expect(error.message).to.be.equal("The second parameter must be an array")
})